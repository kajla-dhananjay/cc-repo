...
docker run ${GITHUB_DOCKER_RUN_ARGS} --entrypoint="" ${DEV_DOCKER_IMAGE_NAME} bash -c "sudo chown -R ${USER}:${USER} ${WORKSPACE_PARENT_DIR} && sudo chown -R ${USER}:docker /var/run/docker.sock && sudo -u ${USER} ${TESTFILES}"
set +ex